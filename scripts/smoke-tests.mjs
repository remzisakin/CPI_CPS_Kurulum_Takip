  ...
  try {
    const result=await protocol.evaluate(`(async()=>{
      ...
      const response={
        ...
        installationCompleted:{stage:completedSaved.workflowStage,status:completedSaved.status,progress:completedSaved.progress,inactive:completedSaved.inactiveWorkPlanIds||[],next:nextServicePlan(completedSaved)?.id||null,completedAt:Boolean(completedSaved.completedAt),visitCompleted:completedSaved.serviceVisits[0]?.completed,planResolved:servicePlanResolved(completedSaved,'p1'),review:completedSaved.completionReview,history:completedSaved.completionReviewHistory}
      };
    ...
    assert.equal(result.installationCompleted.stage,'inService');assert.equal(result.installationCompleted.status,'Devam ediyor');assert.ok(result.installationCompleted.progress<100);assert.deepEqual(result.installationCompleted.inactive,[]);assert.equal(result.installationCompleted.next,'p2');assert.equal(result.installationCompleted.completedAt,false);assert.equal(result.installationCompleted.visitCompleted,true);assert.equal(result.installationCompleted.planResolved,true);assert.equal(result.installationCompleted.review.status,'pending');assert.equal(result.installationCompleted.review.targetVisitId,result.installationCompleted.history[0].targetVisitId);assert.equal(result.installationCompleted.history[0].type,'completion-submitted');
  } finally {
...
test('Faz 7.3B installationCompleted kapanış incelemesi oluşturur ve mevcut plan verisini korur', async () => {
  const fixtureId=-973100;
  await loginAs('Servis Süpervisörü');
  try{
    const result=await protocol.evaluate(`(async()=>{
      const technician=userDirectory.find(user=>user.role==='technician'),originalAskConfirm=askConfirm;askConfirm=async()=>true;
      const item={id:${fixtureId},customer:'COMPLETION SUBMISSION',salesOrderNumber:'REVIEW-973100',workflowStage:'inService',status:'Devam ediyor',progress:45,orderProducts:[],inactiveWorkPlanIds:['legacy-inactive'],installationSchedule:[{id:'review-slot-1',workPlanId:'review-plan-1',date:'2026-09-17',startTime:'09:00',endTime:'11:00',technicians:[technician.name]},{id:'review-slot-2',workPlanId:'review-plan-2',date:'2099-09-18',startTime:'09:00',endTime:'11:00',technicians:[technician.name]}],serviceVisits:[{visitId:'review-first',workPlanId:'review-plan-1',actualVisitDate:'2026-09-16',serviceOutcome:'continuation',continuationPlannedAt:'2026-09-16T12:00:00.000Z',completedWork:'İlk ziyaret',technicians:[technician.name],technicianEntries:[{name:technician.name,travelDuration:0,travelUnit:'Saat',siteDuration:1,siteUnit:'Saat'}]},{visitId:'review-target',workPlanId:'review-plan-1',actualVisitDate:'2026-09-17',serviceOutcome:'continuation',continuationPlannedAt:'2026-09-17T08:00:00.000Z',completedWork:'Önceki ikinci ziyaret',technicians:[technician.name],technicianEntries:[{name:technician.name,travelDuration:0,travelUnit:'Saat',siteDuration:1,siteUnit:'Saat'}]}],completionReviewHistory:[{type:'revision-requested',targetVisitId:'historical-visit',actor:'Supervisor',timestamp:'2026-09-16T15:00:00.000Z'}]};
      const unrelatedVisitBefore=JSON.stringify(item.serviceVisits[0]),scheduleBefore=JSON.stringify(item.installationSchedule),historyBefore=JSON.stringify(item.completionReviewHistory[0]);installations.push(item);saveOperationalData();
      openServiceEntry(item.id);loadServiceVisitV2(item,1,'review-plan-1');const form=document.querySelector('#serviceEntryForm');form.elements.actualVisitDate.value='2026-09-17';form.elements.serviceOutcome.value='installationCompleted';updateServiceFields();form.elements.productStatus.value='complete';form.elements.checklistStatus.value='appropriate';form.elements.completedWork.value='Yeni kapanış beyanı';form.elements.remainingWork.value='';serviceTechnicians=[{name:technician.name,travelDuration:0,travelUnit:'Saat',siteDuration:2,siteUnit:'Saat'}];form.dispatchEvent(new SubmitEvent('submit',{bubbles:true,cancelable:true,submitter:form.querySelector('.primary-button[value="default"]')}));await new Promise(resolve=>setTimeout(resolve,100));
      const saved=operationalRecord(item.id),snapshotBeforeMutation=JSON.stringify(saved.completionReviewHistory.at(-1).snapshot),summary=buildDashboardOperationalSummary([saved],{today:'2026-09-17'}),resolver=resolveOperationalState(saved,{today:'2026-09-17'}),supervisor=buildSupervisorOperationalSummary([saved],{today:'2026-09-17'}),firstSubmission={stage:saved.workflowStage,status:saved.status,progress:saved.progress,completedAt:Boolean(saved.completedAt),review:structuredClone(saved.completionReview),history:structuredClone(saved.completionReviewHistory),targetVisitId:saved.serviceVisits[1].visitId,planResolved:servicePlanResolved(saved,'review-plan-1'),futureActive:servicePlanActive(saved,'review-plan-2'),inactive:[...(saved.inactiveWorkPlanIds||[])],nextPlan:nextServicePlan(saved)?.id||null,unrelatedVisitUnchanged:unrelatedVisitBefore===JSON.stringify(saved.serviceVisits[0]),scheduleUnchanged:scheduleBefore===JSON.stringify(saved.installationSchedule),historyUnchanged:historyBefore===JSON.stringify(saved.completionReviewHistory[0]),todayPlanResolved:summary.todayProgram.find(entry=>entry.planId==='review-plan-1')?.resolved,resolver:{signal:resolver.primarySignal,action:resolver.nextAction,owner:resolver.actionOwnerRole},supervisorPrimary:supervisor.counts.primary};
      saved.serviceVisits[1].completedWork='Gönderimden sonra değiştirildi';firstSubmission.snapshotStable=snapshotBeforeMutation===JSON.stringify(saved.completionReviewHistory.at(-1).snapshot);
      const historyLength=saved.completionReviewHistory.length,reviewBefore=JSON.stringify(saved.completionReview),visitBefore=JSON.stringify(saved.serviceVisits[1]);loadServiceVisitV2(saved,1,'review-plan-1');form.elements.serviceOutcome.value='installationCompleted';form.elements.completedWork.value='İkinci bağımsız gönderim olmamalı';serviceTechnicians=[{name:technician.name,travelDuration:0,travelUnit:'Saat',siteDuration:2,siteUnit:'Saat'}];form.dispatchEvent(new SubmitEvent('submit',{bubbles:true,cancelable:true,submitter:form.querySelector('.primary-button[value="default"]')}));await new Promise(resolve=>setTimeout(resolve,80));firstSubmission.guard={historyLength:saved.completionReviewHistory.length,reviewSame:reviewBefore===JSON.stringify(saved.completionReview),visitSame:visitBefore===JSON.stringify(saved.serviceVisits[1]),originalHistoryLength:historyLength};
      askConfirm=originalAskConfirm;if(document.querySelector('#serviceEntryDialog').open)document.querySelector('#serviceEntryDialog').close();installations=installations.filter(record=>record.id!==item.id);saveOperationalData();render();return firstSubmission;
    })()`);
    assert.equal(result.stage,'inService');assert.equal(result.status,'Devam ediyor');assert.ok(result.progress<100);assert.equal(result.completedAt,false);
    assert.equal(result.review.status,'pending');assert.equal(result.review.targetVisitId,'review-target');assert.equal(result.review.submissionRevision,1);assert.ok(result.review.submittedBy);assert.ok(result.review.submittedAt);
...
test('Completion Review kartı exact target ziyareti gösterir ve karar aksiyonlarını role göre sınırlar', async () => {
  const result=await protocol.evaluate(`(() => {
    const original=currentUser,supervisor=userDirectory.find(user=>user.role==='supervisor'),technician=userDirectory.find(user=>user.role==='technician'),item={id:-975001,revision:4,workflowStage:'inService',orderProducts:[],installationSchedule:[{id:'slot-target',workPlanId:'plan-target',date:'2026-09-17',startTime:'09:00',endTime:'12:00',technicians:[technician.name],activityType:'installation'}],serviceVisits:[{visitId:'wrong-visit',workPlanId:'plan-target',visitNumber:1,actualVisitDate:'2026-09-16',serviceOutcome:'planCompleted',completedWork:'Yanlış ziyaret içeriği',technicians:['Yanlış Teknisyen']},{visitId:'target-visit',workPlanId:'plan-target',visitNumber:2,actualVisitDate:'2026-09-17',serviceOutcome:'installationCompleted',completedWork:'Doğru kapanış içeriği',productStatus:'complete',checklistStatus:'appropriate',technicians:[technician.name],technicianEntries:[{name:technician.name,travelDuration:0,travelUnit:'Saat',siteDuration:3,siteUnit:'Saat'}],generatedReports:[{id:'report-1'}],attachments:[{name:'saha.pdf'}]}],completionReview:{targetVisitId:'target-visit',status:'pending',submissionRevision:2,submittedBy:technician.name,submittedAt:'2026-09-17T12:00:00.000Z'}};
    currentUser=supervisor;const supervisorHtml=completionReviewCardMarkup(item);currentUser=technician;const technicianHtml=completionReviewCardMarkup(item);currentUser=original;
    return{supervisorHasActions:supervisorHtml.includes('data-completion-review-action="approve"')&&supervisorHtml.includes('data-completion-review-action="revision"')&&supervisorHtml.includes('data-completion-review-action="continuation"'),technicianHasActions:technicianHtml.includes('data-completion-review-action='),technicianReadOnly:technicianHtml.includes('completion-review-readonly'),exactTarget:supervisorHtml.includes('Doğru kapanış içeriği')&&!supervisorHtml.includes('Yanlış ziyaret içeriği'),details:supervisorHtml.includes('3 adam-saat')&&supervisorHtml.includes('1 sistem raporu')&&supervisorHtml.includes('1 dosya')};
  })()`);
  assert.deepEqual(result,{supervisorHasActions:true,technicianHasActions:false,technicianReadOnly:true,exactTarget:true,details:true});
});

test('Completion Review presentation onay metnini, gerekçe etiketini ve continuation signalını semantik gösterir', async () => {
  const result=await protocol.evaluate(`(() => {
    const originalLanguage=language;language='tr';const approval=completionReviewApprovalDialogOptions(),reasons=Object.fromEntries(['service-information','insufficient-explanation','missing-technical-document','incorrect-outcome','checklist-information','product-information','work-description','other'].map(value=>[value,completionReviewRevisionReasonLabel(value)])),revisionItem={id:-975005,revision:1,workflowStage:'inService',orderProducts:[],installationSchedule:[{id:'target-slot',workPlanId:'target-plan',date:'2026-09-17',startTime:'09:00',endTime:'10:00',technicians:['Teknisyen A']}],serviceVisits:[{visitId:'target-visit',workPlanId:'target-plan',actualVisitDate:'2026-09-17',serviceOutcome:'installationCompleted',completedWork:'Kapanış kaydı'}],completionReview:{targetVisitId:'target-visit',status:'revisionRequested',submissionRevision:1,submittedBy:'Teknisyen A',submittedAt:'2026-09-17T10:00:00.000Z',decisionCategory:'insufficient-explanation',decisionNote:'Açıklama netleştirilmeli.',decidedBy:'Supervisor',decidedAt:'2026-09-17T11:00:00.000Z'},completionReviewHistory:[{type:'revision-requested',targetVisitId:'target-visit',actor:'Supervisor',timestamp:'2026-09-17T11:00:00.000Z',decisionCategory:'insufficient-explanation',decisionNote:'Açıklama netleştirilmeli.'}]},continuationItem={id:-975006,workflowStage:'inService',status:'Devam planı bekliyor',orderProducts:[],installationSchedule:[{id:'target-slot',workPlanId:'target-plan',date:'2026-09-17',startTime:'09:00',endTime:'10:00',technicians:['Teknisyen A']}],serviceVisits:[{visitId:'target-visit',workPlanId:'target-plan',actualVisitDate:'2026-09-17',serviceOutcome:'installationCompleted'}],pendingContinuationPlanning:true},state=resolveOperationalState(continuationItem,{today:'2026-09-17'}),card=operationalStateCardMarkup(continuationItem),revisionCard=completionReviewCardMarkup(revisionItem),result={approval,reasons,unknown:completionReviewRevisionReasonLabel('legacy-code'),revisionPresentation:{hasLabel:revisionCard.includes('Açıklama yetersiz'),hasCode:revisionCard.includes('insufficient-explanation')},continuation:{rawSignal:state.primarySignal,rawAction:state.nextAction,rawOwner:state.actionOwnerRole,label:operationalStateSignalLabel(state),cardHasAction:card.includes('Devam çalışma planı oluştur'),cardHasOwner:card.includes('Servis Süpervizörü')}};language=originalLanguage;return result;
  })()`);
  assert.deepEqual(result.approval,{title:'Kurulum kapanışını onayla',message:'Onay sonrasında kurulum tamamlanmış olarak kapatılacaktır.',confirmLabel:'Onayla ve Tamamla'});assert.deepEqual(result.reasons,{'service-information':'Eksik/hatalı servis bilgisi','insufficient-explanation':'Açıklama yetersiz','missing-technical-document':'Teknik rapor/belge eksik','incorrect-outcome':'Yanlış çalışma sonucu','checklist-information':'Checklist bilgisi eksik/hatalı','product-information':'Ürün / eksik ürün bilgisi eksik','work-description':'Yapılan/kalan iş açıklaması yetersiz','other':'Diğer'});assert.equal(result.unknown,'Karar gerekçesi kaydedildi');assert.deepEqual(result.revisionPresentation,{hasLabel:true,hasCode:false});assert.deepEqual(result.continuation,{rawSignal:'BLOCKED',rawAction:'CREATE_CONTINUATION_PLAN',rawOwner:'supervisor',label:'Aksiyon Gerekli',cardHasAction:true,cardHasOwner:true});
});

test('Completion Review onayı kapanış tarihini, plan pasifleştirmeyi ve append-only geçmişi korur', async () => {
  const result=await protocol.evaluate(`(() => {
    const slot=(id,date)=>({id:id+'-slot',workPlanId:id,date,startTime:'09:00',endTime:'10:00',technicians:['Teknisyen A']}),history=[{type:'completion-submitted',targetVisitId:'target-visit',actor:'Teknisyen A',timestamp:'2026-09-17T12:00:00.000Z',submissionRevision:3,snapshot:{visitId:'target-visit',completedWork:'Sabit snapshot'}}],base={id:-975010,revision:7,workflowStage:'inService',status:'Devam ediyor',progress:80,orderProducts:[],installationSchedule:[slot('past-plan','2026-09-16'),slot('target-plan','2026-09-17'),slot('future-one','2099-09-18'),slot('future-two','2099-09-19')],serviceVisits:[{visitId:'past-visit',workPlanId:'past-plan',actualVisitDate:'2026-09-16',serviceOutcome:'planCompleted'},{visitId:'target-visit',workPlanId:'target-plan',actualVisitDate:'2026-09-17',serviceOutcome:'installationCompleted'}],inactiveWorkPlanIds:['legacy-inactive','legacy-inactive'],completionReview:{targetVisitId:'target-visit',status:'pending',submissionRevision:3,submittedBy:'Teknisyen A',submittedAt:'2026-09-17T12:00:00.000Z'},completionReviewHistory:history},before=JSON.stringify(base),historyBefore=JSON.stringify(history),approved=applyCompletionReviewDecision(base,'approve',{targetVisitId:'target-visit',submissionRevision:3,actor:'Supervisor A',timestamp:'2026-09-19T08:30:00.000Z'}),blocked=applyCompletionReviewDecision({...base,pendingSalesChangeRequest:{id:'change-1'}},'approve',{targetVisitId:'target-visit',submissionRevision:3,actor:'Supervisor A',timestamp:'2026-09-19T08:30:00.000Z'}),stale=applyCompletionReviewDecision(base,'approve',{targetVisitId:'target-visit',submissionRevision:2,actor:'Supervisor A',timestamp:'2026-09-19T08:30:00.000Z'}),missing=applyCompletionReviewDecision({...base,serviceVisits:base.serviceVisits.filter(visit=>visit.visitId!=='target-visit')},'approve',{targetVisitId:'target-visit',submissionRevision:3,actor:'Supervisor A',timestamp:'2026-09-19T08:30:00.000Z'}),second=approved.ok?applyCompletionReviewDecision(approved.item,'approve',{targetVisitId:'target-visit',submissionRevision:3,actor:'Supervisor B',timestamp:'2026-09-19T08:31:00.000Z'}):null;
    const targetChanged=applyCompletionReviewDecision(base,'approve',{targetVisitId:'previous-target',submissionRevision:3,actor:'Supervisor A',timestamp:'2026-09-19T08:30:00.000Z'});
    return{sourceUnchanged:before===JSON.stringify(base),historyUnchanged:historyBefore===JSON.stringify(history),approved:approved.ok&&{stage:approved.item.workflowStage,status:approved.item.status,progress:approved.item.progress,completedAt:approved.item.completedAt,approvedAt:approved.item.completionApprovedAt,pending:approved.item.pendingContinuationPlanning,hasReview:Boolean(approved.item.completionReview),inactive:approved.item.inactiveWorkPlanIds,historyLength:approved.item.completionReviewHistory.length,last:approved.item.completionReviewHistory.at(-1),pastStillActive:servicePlanActive(approved.item,'past-plan'),targetResolved:servicePlanResolved(approved.item,'target-plan')},blocked:blocked.code,stale:stale.code,targetChanged:targetChanged.code,missing:missing.code,second:second?.code};
  })()`);
  assert.equal(result.sourceUnchanged,true);assert.equal(result.historyUnchanged,true);assert.deepEqual(result.approved,{stage:'completed',status:'Tamamlandı',progress:100,completedAt:'2026-09-17',approvedAt:'2026-09-19T08:30:00.000Z',pending:false,hasReview:false,inactive:['legacy-inactive','future-one','future-two'],historyLength:2,last:{type:'completion-approved',targetVisitId:'target-visit',actor:'Supervisor A',timestamp:'2026-09-19T08:30:00.000Z',submissionRevision:3},pastStillActive:true,targetResolved:true});assert.equal(result.blocked,'PENDING_SALES_CHANGE');assert.equal(result.stale,'SUBMISSION_CHANGED');assert.equal(result.targetChanged,'TARGET_CHANGED');assert.equal(result.missing,'TARGET_INVALID');assert.equal(result.second,'REVIEW_UNAVAILABLE');
});

test('Completion Review düzeltme ve devam kararları plan/workflow semantiğini korur', async () => {
  const result=await protocol.evaluate(`(() => {
    const slot=(id,date)=>({id:id+'-slot',workPlanId:id,date,startTime:'09:00',endTime:'10:00',technicians:['Teknisyen A']}),make=(future=true)=>({id:future?-975020:-975021,revision:2,workflowStage:'inService',status:'Devam ediyor',progress:75,orderProducts:[],installationSchedule:[slot('target-plan','2026-09-17'),...(future?[slot('future-plan','2099-09-18')]:[])],serviceVisits:[{visitId:'target-visit',workPlanId:'target-plan',actualVisitDate:'2026-09-17',serviceOutcome:'installationCompleted'}],completionReview:{targetVisitId:'target-visit',status:'pending',submissionRevision:1,submittedBy:'Teknisyen A',submittedAt:'2026-09-17T12:00:00.000Z'},completionReviewHistory:[{type:'completion-submitted',targetVisitId:'target-visit',actor:'Teknisyen A',timestamp:'2026-09-17T12:00:00.000Z',submissionRevision:1,snapshot:{visitId:'target-visit'}}]}),revision=applyCompletionReviewDecision(make(),'revision',{targetVisitId:'target-visit',submissionRevision:1,decisionCategory:'service-information',decisionNote:'Servis açıklamasını tamamlayın.',actor:'Supervisor A',timestamp:'2026-09-17T13:00:00.000Z'}),invalidRevision=applyCompletionReviewDecision(make(),'revision',{targetVisitId:'target-visit',submissionRevision:1,actor:'Supervisor A',timestamp:'2026-09-17T13:00:00.000Z'}),withFuture=applyCompletionReviewDecision(make(),'continuation',{targetVisitId:'target-visit',submissionRevision:1,decisionNote:'Ek saha testi gerekli.',actor:'Supervisor A',timestamp:'2026-09-17T13:10:00.000Z'}),withoutFuture=applyCompletionReviewDecision(make(false),'continuation',{targetVisitId:'target-visit',submissionRevision:1,decisionNote:'Yeni ziyaret planlanmalı.',actor:'Supervisor A',timestamp:'2026-09-17T13:20:00.000Z'}),invalidContinuation=applyCompletionReviewDecision(make(false),'continuation',{targetVisitId:'target-visit',submissionRevision:1,actor:'Supervisor A',timestamp:'2026-09-17T13:20:00.000Z'}),futureState=resolveOperationalState(withFuture.item,{today:'2026-09-17'}),noFutureState=resolveOperationalState(withoutFuture.item,{today:'2026-09-17'});
    return{revision:{ok:revision.ok,status:revision.item.completionReview.status,category:revision.item.completionReview.decisionCategory,note:revision.item.completionReview.decisionNote,stage:revision.item.workflowStage,completedAt:revision.item.completedAt||null,targetResolved:servicePlanResolved(revision.item,'target-plan'),futureActive:servicePlanActive(revision.item,'future-plan'),history:revision.item.completionReviewHistory.at(-1)},invalidRevision:invalidRevision.code,withFuture:{pending:withFuture.item.pendingContinuationPlanning,review:Boolean(withFuture.item.completionReview),futureActive:servicePlanActive(withFuture.item,'future-plan'),targetResolved:servicePlanResolved(withFuture.item,'target-plan'),state:{signal:futureState.primarySignal,action:futureState.nextAction},history:withFuture.item.completionReviewHistory.at(-1)},withoutFuture:{pending:withoutFuture.item.pendingContinuationPlanning,review:Boolean(withoutFuture.item.completionReview),targetResolved:servicePlanResolved(withoutFuture.item,'target-plan'),state:{signal:noFutureState.primarySignal,action:noFutureState.nextAction,owner:noFutureState.actionOwnerRole},history:withoutFuture.item.completionReviewHistory.at(-1)},invalidContinuation:invalidContinuation.code};
  })()`);
  assert.deepEqual(result.revision,{ok:true,status:'revisionRequested',category:'service-information',note:'Servis açıklamasını tamamlayın.',stage:'inService',completedAt:null,targetResolved:true,futureActive:true,history:{type:'revision-requested',targetVisitId:'target-visit',actor:'Supervisor A',timestamp:'2026-09-17T13:00:00.000Z',submissionRevision:1,decisionCategory:'service-information',decisionNote:'Servis açıklamasını tamamlayın.'}});assert.equal(result.invalidRevision,'REVISION_REASON_REQUIRED');assert.deepEqual(result.withFuture,{pending:false,review:false,futureActive:true,targetResolved:true,state:{signal:'WAITING',action:'NONE'},history:{type:'continuation-required',targetVisitId:'target-visit',actor:'Supervisor A',timestamp:'2026-09-17T13:10:00.000Z',submissionRevision:1,decisionNote:'Ek saha testi gerekli.'}});assert.deepEqual(result.withoutFuture,{pending:true,review:false,targetResolved:true,state:{signal:'BLOCKED',action:'CREATE_CONTINUATION_PLAN',owner:'supervisor'},history:{type:'continuation-required',targetVisitId:'target-visit',actor:'Supervisor A',timestamp:'2026-09-17T13:20:00.000Z',submissionRevision:1,decisionNote:'Yeni ziyaret planlanmalı.'}});assert.equal(result.invalidContinuation,'CONTINUATION_NOTE_REQUIRED');
});

test('Completion Review revision loop aynı ziyareti immutable geçmişle tekrar gönderir', async () => {
  const result=await protocol.evaluate(`(() => {
    const slot=(id,date)=>({id:id+'-slot',workPlanId:id,date,startTime:'09:00',endTime:'11:00',technicians:['Teknisyen A']}),history=[{type:'completion-submitted',targetVisitId:'target-visit',actor:'Teknisyen A',timestamp:'2026-09-17T10:00:00.000Z',submissionRevision:2,snapshot:{visitId:'target-visit',completedWork:'İlk metin'}},{type:'revision-requested',targetVisitId:'target-visit',actor:'Supervisor A',timestamp:'2026-09-17T11:00:00.000Z',submissionRevision:2,decisionCategory:'work-description',decisionNote:'Yapılan işi netleştirin.'}],base={id:-975030,revision:5,workflowStage:'inService',status:'Devam ediyor',progress:80,pendingSalesChangeRequest:{id:'change-does-not-block-resubmit'},orderProducts:[],installationSchedule:[slot('target-plan','2026-09-17'),slot('future-plan','2099-09-18')],serviceVisits:[{visitId:'other-visit',workPlanId:'target-plan',actualVisitDate:'2026-09-16',serviceOutcome:'continuation',completedWork:'Dokunulmayacak ziyaret'},{visitId:'target-visit',workPlanId:'target-plan',actualVisitDate:'2026-09-17',serviceOutcome:'installationCompleted',completedWork:'İlk metin',productStatus:'complete',checklistStatus:'appropriate',technicians:['Teknisyen A'],technicianEntries:[{name:'Teknisyen A',travelDuration:0,travelUnit:'Saat',siteDuration:2,siteUnit:'Saat'}]}],completionReview:{targetVisitId:'target-visit',status:'revisionRequested',submissionRevision:2,submittedBy:'Teknisyen A',submittedAt:'2026-09-17T10:00:00.000Z',decisionCategory:'work-description',decisionNote:'Yapılan işi netleştirin.',decidedBy:'Supervisor A',decidedAt:'2026-09-17T11:00:00.000Z'},completionReviewHistory:history},before=JSON.stringify(base),historyBefore=JSON.stringify(history),updated={...base.serviceVisits[1],completedWork:'Düzeltilmiş ayrıntılı açıklama'},resubmitted=applyCompletionReviewResubmission(base,updated,{targetVisitId:'target-visit',submissionRevision:2,actor:'Teknisyen A',timestamp:'2026-09-17T12:00:00.000Z'}),stale=applyCompletionReviewResubmission(base,updated,{targetVisitId:'target-visit',submissionRevision:1,actor:'Teknisyen A',timestamp:'2026-09-17T12:00:00.000Z'}),wrongTarget=applyCompletionReviewResubmission(base,updated,{targetVisitId:'other-visit',submissionRevision:2,actor:'Teknisyen A',timestamp:'2026-09-17T12:00:00.000Z'}),wrongVisit=applyCompletionReviewResubmission(base,{...updated,visitId:'replacement-visit'},{targetVisitId:'target-visit',submissionRevision:2,actor:'Teknisyen A',timestamp:'2026-09-17T12:00:00.000Z'}),wrongPlan=applyCompletionReviewResubmission(base,{...updated,workPlanId:'future-plan'},{targetVisitId:'target-visit',submissionRevision:2,actor:'Teknisyen A',timestamp:'2026-09-17T12:00:00.000Z'}),wrongOutcome=applyCompletionReviewResubmission(base,{...updated,serviceOutcome:'planCompleted'},{targetVisitId:'target-visit',submissionRevision:2,actor:'Teknisyen A',timestamp:'2026-09-17T12:00:00.000Z'}),second=resubmitted.ok?applyCompletionReviewResubmission(resubmitted.item,updated,{targetVisitId:'target-visit',submissionRevision:2,actor:'Teknisyen A',timestamp:'2026-09-17T12:01:00.000Z'}):null,state=resubmitted.ok?resolveOperationalState(resubmitted.item,{today:'2026-09-17'}):null;
    if(resubmitted.ok)resubmitted.item.serviceVisits[1].completedWork='Sonradan değiştirildi';
    return{sourceUnchanged:before===JSON.stringify(base),historyUnchanged:historyBefore===JSON.stringify(history),ok:resubmitted.ok,review:resubmitted.ok&&resubmitted.item.completionReview,historyLength:resubmitted.ok&&resubmitted.item.completionReviewHistory.length,last:resubmitted.ok&&resubmitted.item.completionReviewHistory.at(-1),visitCount:resubmitted.ok&&resubmitted.item.serviceVisits.length,otherVisit:resubmitted.ok&&resubmitted.item.serviceVisits[0].completedWork,targetIdentity:resubmitted.ok&&[resubmitted.item.serviceVisits[1].visitId,resubmitted.item.serviceVisits[1].workPlanId,resubmitted.item.serviceVisits[1].serviceOutcome],futureActive:resubmitted.ok&&servicePlanActive(resubmitted.item,'future-plan'),state:state&&{signal:state.primarySignal,action:state.nextAction,owner:state.actionOwnerRole},snapshotStable:resubmitted.ok&&resubmitted.item.completionReviewHistory.at(-1).snapshot.completedWork==='Düzeltilmiş ayrıntılı açıklama',stale:stale.code,wrongTarget:wrongTarget.code,wrongVisit:wrongVisit.code,wrongPlan:wrongPlan.code,wrongOutcome:wrongOutcome.code,second:second?.code};
  })()`);
  assert.equal(result.sourceUnchanged,true);assert.equal(result.historyUnchanged,true);assert.equal(result.ok,true);assert.deepEqual(result.review,{targetVisitId:'target-visit',status:'pending',submissionRevision:3,submittedBy:'Teknisyen A',submittedAt:'2026-09-17T12:00:00.000Z',decisionCategory:'',decisionNote:'',decidedBy:'',decidedAt:''});assert.equal(result.historyLength,3);assert.equal(result.last.type,'completion-resubmitted');assert.equal(result.last.submissionRevision,3);assert.equal(result.last.snapshot.completedWork,'Düzeltilmiş ayrıntılı açıklama');assert.equal(result.snapshotStable,true);assert.equal(result.visitCount,2);assert.equal(result.otherVisit,'Dokunulmayacak ziyaret');assert.deepEqual(result.targetIdentity,['target-visit','target-plan','installationCompleted']);assert.equal(result.futureActive,true);assert.deepEqual(result.state,{signal:'ACTION_REQUIRED',action:'REVIEW_COMPLETION_SUBMISSION',owner:'supervisor'});assert.equal(result.stale,'SUBMISSION_CHANGED');assert.equal(result.wrongTarget,'TARGET_CHANGED');assert.equal(result.wrongVisit,'VISIT_ID_CHANGED');assert.equal(result.wrongPlan,'WORK_PLAN_CHANGED');assert.equal(result.wrongOutcome,'OUTCOME_CHANGED');assert.equal(result.second,'REVISION_NOT_REQUESTED');
});

test('Completion Review revision 2 sonrasi yeniden duzeltme, onay ve devam kararlari korunur', async () => {
  const result=await protocol.evaluate(`(() => {
    const base={id:-975031,workflowStage:'inService',status:'Devam ediyor',progress:80,orderProducts:[],installationSchedule:[{id:'target-slot',workPlanId:'target-plan',date:'2026-09-17',startTime:'09:00',endTime:'11:00',technicians:['Teknisyen A']}],serviceVisits:[{visitId:'target-visit',workPlanId:'target-plan',actualVisitDate:'2026-09-17',serviceOutcome:'installationCompleted',completedWork:'Revision 1'}],completionReview:{targetVisitId:'target-visit',status:'revisionRequested',submissionRevision:1,submittedBy:'Teknisyen A',submittedAt:'2026-09-17T09:00:00.000Z',decisionCategory:'work-description',decisionNote:'Detay ekleyin.',decidedBy:'Supervisor A',decidedAt:'2026-09-17T10:00:00.000Z'},completionReviewHistory:[{type:'completion-submitted',targetVisitId:'target-visit',actor:'Teknisyen A',timestamp:'2026-09-17T09:00:00.000Z',submissionRevision:1,snapshot:{visitId:'target-visit',completedWork:'Revision 1'}},{type:'revision-requested',targetVisitId:'target-visit',actor:'Supervisor A',timestamp:'2026-09-17T10:00:00.000Z',submissionRevision:1,decisionCategory:'work-description',decisionNote:'Detay ekleyin.'}]},revision2=applyCompletionReviewResubmission(base,{...base.serviceVisits[0],completedWork:'Revision 2'},{targetVisitId:'target-visit',submissionRevision:1,actor:'Teknisyen A',timestamp:'2026-09-17T11:00:00.000Z'}),requestedAgain=applyCompletionReviewDecision(revision2.item,'revision',{targetVisitId:'target-visit',submissionRevision:2,decisionCategory:'service-information',decisionNote:'Bir alan daha duzeltilmeli.',actor:'Supervisor A',timestamp:'2026-09-17T12:00:00.000Z'}),revision3=applyCompletionReviewResubmission(requestedAgain.item,{...requestedAgain.item.serviceVisits[0],completedWork:'Revision 3'},{targetVisitId:'target-visit',submissionRevision:2,actor:'Teknisyen A',timestamp:'2026-09-17T13:00:00.000Z'}),approved=applyCompletionReviewDecision(revision2.item,'approve',{targetVisitId:'target-visit',submissionRevision:2,actor:'Supervisor A',timestamp:'2026-09-17T12:30:00.000Z'}),continued=applyCompletionReviewDecision(revision2.item,'continuation',{targetVisitId:'target-visit',submissionRevision:2,decisionNote:'Ek ziyaret gerekli.',actor:'Supervisor A',timestamp:'2026-09-17T12:40:00.000Z'});
    return{revision2:{revision:revision2.item.completionReview.submissionRevision,types:revision2.item.completionReviewHistory.map(entry=>entry.type),snapshots:revision2.item.completionReviewHistory.filter(entry=>entry.snapshot).map(entry=>entry.snapshot.completedWork)},revision3:{revision:revision3.item.completionReview.submissionRevision,types:revision3.item.completionReviewHistory.map(entry=>entry.type),snapshots:revision3.item.completionReviewHistory.filter(entry=>entry.snapshot).map(entry=>entry.snapshot.completedWork)},approved:{stage:approved.item.workflowStage,status:approved.item.status,progress:approved.item.progress,review:Boolean(approved.item.completionReview),last:approved.item.completionReviewHistory.at(-1).type},continued:{stage:continued.item.workflowStage,review:Boolean(continued.item.completionReview),pending:continued.item.pendingContinuationPlanning,last:continued.item.completionReviewHistory.at(-1).type}};
  })()`);
  assert.deepEqual(result.revision2,{revision:2,types:['completion-submitted','revision-requested','completion-resubmitted'],snapshots:['Revision 1','Revision 2']});assert.deepEqual(result.revision3,{revision:3,types:['completion-submitted','revision-requested','completion-resubmitted','revision-requested','completion-resubmitted'],snapshots:['Revision 1','Revision 2','Revision 3']});assert.deepEqual(result.approved,{stage:'completed',status:'Tamamlandı',progress:100,review:false,last:'completion-approved'});assert.deepEqual(result.continued,{stage:'inService',review:false,pending:true,last:'continuation-required'});
});

test('Completion Review revision form exact target ziyareti yerinde günceller ve yetkiyi sınırlar', async () => {
  const fixtureId=-975032;
  const result=await protocol.evaluate(`(async()=>{
    const originalUser=currentUser,technician=userDirectory.find(user=>user.role==='technician'),supervisor=userDirectory.find(user=>user.role==='supervisor'),admin=userDirectory.find(user=>user.role==='admin'),item={id:${fixtureId},revision:1,customer:'REVISION LOOP',salesOrderNumber:'REVIEW-973E',workflowStage:'inService',status:'Devam ediyor',progress:80,pendingSalesChangeRequest:{id:'sales-change-kept'},orderProducts:[],installationSchedule:[{id:'target-slot',workPlanId:'target-plan',date:'2026-09-17',startTime:'09:00',endTime:'11:00',technicians:[technician.name],activityType:'installation',productControl:'required',checklistRequirement:'required'}],serviceVisits:[{visitId:'untouched-visit',workPlanId:'target-plan',actualVisitDate:'2026-09-16',serviceOutcome:'continuation',completedWork:'Değişmemeli',technicians:[technician.name],technicianEntries:[{name:technician.name,travelDuration:0,travelUnit:'Saat',siteDuration:1,siteUnit:'Saat'}]},{visitId:'target-visit',workPlanId:'target-plan',actualVisitDate:'2026-09-17',serviceOutcome:'installationCompleted',completedWork:'Eski açıklama',productStatus:'complete',checklistStatus:'appropriate',technicians:[technician.name],technicianEntries:[{name:technician.name,travelDuration:0,travelUnit:'Saat',siteDuration:2,siteUnit:'Saat'}]}],completionReview:{targetVisitId:'target-visit',status:'revisionRequested',submissionRevision:2,submittedBy:technician.name,submittedAt:'2026-09-17T10:00:00.000Z',decisionCategory:'work-description',decisionNote:'Yapılan işi ayrıntılandırın.',decidedBy:supervisor.name,decidedAt:'2026-09-17T11:00:00.000Z'},completionReviewHistory:[{type:'completion-submitted',targetVisitId:'target-visit',actor:technician.name,timestamp:'2026-09-17T10:00:00.000Z',submissionRevision:2,snapshot:{visitId:'target-visit',completedWork:'Eski açıklama'}},{type:'revision-requested',targetVisitId:'target-visit',actor:supervisor.name,timestamp:'2026-09-17T11:00:00.000Z',submissionRevision:2,decisionCategory:'work-description',decisionNote:'Yapılan işi ayrıntılandırın.'}]};
    currentUser=technician;const technicianAllowed=completionReviewRevisionAuthorized(item),technicianCta=completionReviewCardMarkup(item).includes('data-completion-review-edit');currentUser=supervisor;const supervisorAllowed=completionReviewRevisionAuthorized(item),supervisorCta=completionReviewCardMarkup(item).includes('data-completion-review-edit');currentUser=admin;const adminAllowed=completionReviewRevisionAuthorized(item);currentUser=technician;installations.push(item);saveOperationalData();openCompletionReviewRevision({dataset:{installationId:String(item.id),targetVisitId:'target-visit',submissionRevision:'2',baseRevision:'1'}});const form=document.querySelector('#serviceEntryForm'),dialog=document.querySelector('#serviceEntryDialog'),opened={open:dialog.open,mode:form.dataset.completionRevisionMode,index:form.elements.visitIndex.value,plan:form.elements.workPlanId.value,planLocked:document.querySelector('#serviceWorkPlanSelect').disabled,outcome:form.elements.serviceOutcome.value,outcomeLocked:form.elements.serviceOutcome.disabled,title:dialog.querySelector('.dialog-header h2').textContent,button:form.querySelector('.primary-button[value="default"]').textContent,reason:document.querySelector('#serviceRevisionContext').textContent};form.elements.completedWork.value='Düzeltilmiş saha açıklaması';form.dispatchEvent(new SubmitEvent('submit',{bubbles:true,cancelable:true,submitter:form.querySelector('.primary-button[value="default"]')}));await new Promise(resolve=>setTimeout(resolve,120));const saved=operationalRecord(item.id),state=resolveOperationalState(saved,{today:'2026-09-17'}),response={authorization:{technicianAllowed,supervisorAllowed,adminAllowed,technicianCta,supervisorCta},opened,visitCount:saved.serviceVisits.length,untouched:saved.serviceVisits[0].completedWork,target:{id:saved.serviceVisits[1].visitId,plan:saved.serviceVisits[1].workPlanId,outcome:saved.serviceVisits[1].serviceOutcome,work:saved.serviceVisits[1].completedWork},review:saved.completionReview,history:saved.completionReviewHistory.at(-1),pendingSalesChange:Boolean(saved.pendingSalesChangeRequest),state:{signal:state.primarySignal,action:state.nextAction,owner:state.actionOwnerRole},reset:{mode:form.dataset.completionRevisionMode||'',contextHidden:document.querySelector('#serviceRevisionContext').classList.contains('role-hidden')}};if(dialog.open)dialog.close();if(document.querySelector('#installationDetailDialog').open)document.querySelector('#installationDetailDialog').close();installations=installations.filter(record=>record.id!==item.id);saveOperationalData();currentUser=originalUser;render();return response;
  })()`);
  assert.deepEqual(result.authorization,{technicianAllowed:true,supervisorAllowed:false,adminAllowed:true,technicianCta:true,supervisorCta:false});assert.equal(result.opened.open,true);assert.equal(result.opened.mode,'true');assert.equal(result.opened.index,'1');assert.equal(result.opened.plan,'target-plan');assert.equal(result.opened.planLocked,true);assert.equal(result.opened.outcome,'installationCompleted');assert.equal(result.opened.outcomeLocked,true);assert.equal(result.opened.title,'Kapanış Kaydını Düzelt');assert.equal(result.opened.button,'Düzelt ve Tekrar Gönder');assert.ok(result.opened.reason.includes('Yapılan işi ayrıntılandırın.'));assert.equal(result.visitCount,2);assert.equal(result.untouched,'Değişmemeli');assert.deepEqual(result.target,{id:'target-visit',plan:'target-plan',outcome:'installationCompleted',work:'Düzeltilmiş saha açıklaması'});assert.equal(result.review.status,'pending');assert.equal(result.review.submissionRevision,3);assert.equal(result.history.type,'completion-resubmitted');assert.equal(result.history.snapshot.completedWork,'Düzeltilmiş saha açıklaması');assert.equal(result.pendingSalesChange,true);assert.deepEqual(result.state,{signal:'ACTION_REQUIRED',action:'REVIEW_COMPLETION_SUBMISSION',owner:'supervisor'});assert.deepEqual(result.reset,{mode:'',contextHidden:true});
});

test('Service outcome alanları revision ve normal modda doğru görünür', async () => {
  const result=await protocol.evaluate(`(() => {
    const form=document.querySelector('#serviceEntryForm'), continuation=document.querySelector('#serviceContinuationFields'), blocked=document.querySelector('#serviceCouldNotPerformFields');
    const state=outcome=>{document.querySelector('#serviceOutcome').innerHTML=\`<option value="\${outcome}" selected>\${outcome}</option>\`;updateServiceFieldsV5();return{continuationHidden:continuation.classList.contains('role-hidden'),continuationRequired:[...continuation.querySelectorAll('input,select,textarea')].some(field=>field.required),blockedHidden:blocked.classList.contains('role-hidden'),blockedRequired:[...blocked.querySelectorAll('input,select,textarea')].some(field=>field.required)}};
    return{installationCompleted:state('installationCompleted'),couldNotPerform:state('couldNotPerform'),continuation:state('continuation'),partialCompleted:state('partialCompleted')};
  })()`);
  assert.deepEqual(result.installationCompleted,{continuationHidden:true,continuationRequired:false,blockedHidden:true,blockedRequired:false});
  assert.deepEqual(result.couldNotPerform,{continuationHidden:true,continuationRequired:false,blockedHidden:false,blockedRequired:true});
  assert.deepEqual(result.continuation,{continuationHidden:false,continuationRequired:true,blockedHidden:true,blockedRequired:false});
  assert.deepEqual(result.partialCompleted,{continuationHidden:false,continuationRequired:true,blockedHidden:true,blockedRequired:false});
});

test('Faz 7.3F Scenario A explicit source ilişkisini tek continuation plana yazar', async () => {
  const result=await protocol.evaluate(`(() => {
    const base={id:-975040,workflowStage:'inService',status:'Devam planı bekliyor',pendingContinuationPlanning:true,installationSchedule:[{id:'target-slot',workPlanId:'target-plan',date:'2026-09-17',startTime:'09:00',endTime:'11:00',technicians:['Teknisyen A']}],serviceVisits:[{visitId:'source-visit',workPlanId:'target-plan',actualVisitDate:'2026-09-17',serviceOutcome:'installationCompleted'}],completionReviewHistory:[{type:'continuation-required',targetVisitId:'source-visit',actor:'Supervisor A',timestamp:'2026-09-17T12:00:00.000Z',decisionNote:'Ek saha çalışması gerekli.'}]},newSlots=[...base.installationSchedule,{id:'new-a',workPlanId:'continuation-plan',sourceVisitId:'source-visit',date:'2099-09-20',startTime:'09:00',endTime:'11:00',technicians:['Teknisyen B']},{id:'new-b',workPlanId:'continuation-plan',sourceVisitId:'source-visit',date:'2099-09-20',startTime:'11:00',endTime:'13:00',technicians:['Teknisyen C']}],validation=validateContinuationPlanRelations(base,newSlots),next=structuredClone(base);next.installationSchedule=newSlots;const applied=applyContinuationPlanRelations(next,validation,'2026-09-17T13:00:00.000Z'),plan=serviceWorkPlans(next).find(entry=>entry.id==='continuation-plan'),state=resolveOperationalState(next,{today:'2026-09-17'}),normal=validateContinuationPlanRelations({...base,completionReviewHistory:[]},base.installationSchedule),ambiguous=validateContinuationPlanRelations(base,[...base.installationSchedule,{id:'a',workPlanId:'bad-plan',sourceVisitId:'source-visit',date:'2099-09-20',startTime:'09:00',endTime:'10:00',technicians:['A']},{id:'b',workPlanId:'bad-plan',sourceVisitId:'other-visit',date:'2099-09-20',startTime:'10:00',endTime:'11:00',technicians:['B']}]);return{context:continuationPlanningContext(base)?.visit?.visitId,valid:validation.ok,applied,slotSources:plan.slots.map(slot=>slot.sourceVisitId),resolved:resolveContinuationSourceVisit(next,plan)?.visitId,pending:next.pendingContinuationPlanning,plannedAt:next.serviceVisits[0].continuationPlannedAt,state:{signal:state.primarySignal,action:state.nextAction},normal:{ok:normal.ok,size:normal.sources.size},ambiguous:ambiguous.code};})()`);
  assert.deepEqual(result,{context:'source-visit',valid:true,applied:true,slotSources:['source-visit','source-visit'],resolved:'source-visit',pending:false,plannedAt:'2026-09-17T13:00:00.000Z',state:{signal:'WAITING',action:'NONE'},normal:{ok:true,size:0},ambiguous:'AMBIGUOUS_SOURCE'});
});

test('Faz 7.3F Scenario B future planı otomatik seçmez ve explicit seçim veriyi korur', async () => {
  const result=await protocol.evaluate(`(() => {
    const target={id:-975041,workflowStage:'inService',status:'Devam ediyor',pendingContinuationPlanning:false,installationSchedule:[{id:'target-slot',workPlanId:'target-plan',date:'2026-09-17',startTime:'09:00',endTime:'11:00',technicians:['Teknisyen A']},{id:'future-slot',workPlanId:'future-plan',workPlanNote:'Mevcut plan notu',date:'2099-09-21',startTime:'10:00',endTime:'12:30',technicians:['Teknisyen B']},{id:'other-future',workPlanId:'other-plan',date:'2099-09-22',startTime:'13:00',endTime:'15:00',technicians:['Teknisyen C']}],serviceVisits:[{visitId:'source-visit',workPlanId:'target-plan',actualVisitDate:'2026-09-17',serviceOutcome:'installationCompleted'}],completionReviewHistory:[{type:'continuation-required',targetVisitId:'source-visit',actor:'Supervisor A',timestamp:'2026-09-17T12:00:00.000Z',decisionNote:'Devam edin.'}]},context=continuationPlanningContext(target),before=JSON.stringify(target.installationSchedule.find(slot=>slot.workPlanId==='future-plan')),proposed=target.installationSchedule.map(slot=>slot.workPlanId==='future-plan'?{...slot,sourceVisitId:'source-visit'}:{...slot}),validation=validateContinuationPlanRelations(target,proposed),next=structuredClone(target);next.installationSchedule=proposed;applyContinuationPlanRelations(next,validation,'2026-09-17T13:00:00.000Z');const selected=next.installationSchedule.find(slot=>slot.workPlanId==='future-plan'),unrelated=next.installationSchedule.find(slot=>slot.workPlanId==='other-plan'),afterWithoutSource=JSON.stringify({...selected,sourceVisitId:undefined}),beforeWithoutSource=JSON.stringify({...JSON.parse(before),sourceVisitId:undefined}),duplicate=validateContinuationPlanRelations(target,[...proposed,{id:'duplicate-slot',workPlanId:'duplicate-plan',sourceVisitId:'source-visit',date:'2099-09-23',startTime:'09:00',endTime:'10:00',technicians:['Teknisyen D']}]),occupied={...target,serviceVisits:[...target.serviceVisits,{visitId:'other-source',workPlanId:'target-plan',serviceOutcome:'installationCompleted'}],completionReviewHistory:[...target.completionReviewHistory,{type:'continuation-required',targetVisitId:'other-source',actor:'Supervisor A',timestamp:'2026-09-17T12:30:00.000Z',decisionNote:'Başka devam.'}],installationSchedule:target.installationSchedule.map(slot=>slot.workPlanId==='future-plan'?{...slot,sourceVisitId:'other-source'}:{...slot})},overwrite=validateContinuationPlanRelations(occupied,occupied.installationSchedule.map(slot=>slot.workPlanId==='future-plan'?{...slot,sourceVisitId:'source-visit'}:{...slot}));return{pendingContext:context?.pending,source:context?.visit?.visitId,futureIds:continuationFuturePlans(target).map(plan=>plan.id),autoSelected:target.installationSchedule.some(slot=>slot.sourceVisitId),valid:validation.ok,preserved:beforeWithoutSource===afterWithoutSource,selectedSource:selected.sourceVisitId,unrelatedSource:unrelated.sourceVisitId||'',duplicate:duplicate.code,overwrite:overwrite.code};})()`);
  assert.deepEqual(result,{pendingContext:true,source:'source-visit',futureIds:['future-plan','other-plan'],autoSelected:false,valid:true,preserved:true,selectedSource:'source-visit',unrelatedSource:'',duplicate:'SOURCE_ALREADY_LINKED',overwrite:'PLAN_ALREADY_LINKED'});
});

test('Completion Review multi-continuation döngüsü her ardışık installationCompleted ziyaretini yeniden inceler', async () => {
  const fixtureId=-975060;
  await loginAs('Servis Süpervisörü');
  try {
    const result=await protocol.evaluate(`(async()=>{
      const technician=userDirectory.find(user=>user.role==='technician'),originalAskConfirm=askConfirm;askConfirm=async()=>true;
      const slot=(planId,date)=>({id:planId+'-slot',workPlanId:planId,date,startTime:'09:00',endTime:'11:00',technicians:[technician.name],activityType:'installation',productControl:'required',checklistRequirement:'required'});
      const item={id:${fixtureId},customer:'MULTI CONTINUATION',salesOrderNumber:'REVIEW-975060',workflowStage:'inService',status:'Devam ediyor',progress:60,orderProducts:[],installationSchedule:[slot('plan-1','2026-09-17')],serviceVisits:[]};
      installations.push(item);saveOperationalData();
      const submitVisit=async(planId,date)=>{if(planId!=='plan-1'){const current=operationalRecord(item.id),visit={visitId:'visit-'+planId,workPlanId:planId,actualVisitDate:date,serviceOutcome:'installationCompleted',completedWork:'Tamamlanan işler',technicians:[technician.name]};current.serviceVisits=[...(current.serviceVisits||[]),visit];current.completionReview=normalizeCompletionReview({targetVisitId:visit.visitId,status:'pending',submissionRevision:1,submittedBy:technician.name,submittedAt:date+'T12:00:00.000Z'});current.completionReviewHistory=appendCompletionReviewHistory(current.completionReviewHistory,{type:'completion-submitted',targetVisitId:visit.visitId,actor:technician.name,timestamp:date+'T12:00:00.000Z',submissionRevision:1,snapshot:createCompletionSubmissionSnapshot(visit)});Object.assign(item,current);saveOperationalData();return operationalRecord(item.id)}openServiceEntry(item.id);loadServiceWorkPlan(operationalRecord(item.id),planId);const form=document.querySelector('#serviceEntryForm');form.elements.actualVisitDate.value=date;form.elements.serviceOutcome.value='installationCompleted';updateServiceFields();form.elements.productStatus.value='complete';form.elements.checklistStatus.value='appropriate';form.elements.completedWork.value='Tamamlanan işler';form.elements.remainingWork.value='';serviceTechnicians=[{name:technician.name,travelDuration:0,travelUnit:'Saat',siteDuration:1,siteUnit:'Saat'}];form.dispatchEvent(new SubmitEvent('submit',{bubbles:true,cancelable:true,submitter:form.querySelector('.primary-button[value="default"]')}));await new Promise(resolve=>setTimeout(resolve,80));if(document.querySelector('#serviceEntryDialog').open)document.querySelector('#serviceEntryDialog').close();return operationalRecord(item.id);};
      const continuation=async(visitId)=>{const current=operationalRecord(item.id),review=current.completionReview,decision=applyCompletionReviewDecision(current,'continuation',{targetVisitId:visitId,submissionRevision:review.submissionRevision,decisionNote:'Devam gerekli.',actor:'Supervisor',timestamp:'2026-09-17T12:00:00.000Z'});if(!decision.ok)throw new Error('Continuation decision failed: '+decision.code);Object.assign(item,decision.item);if(!decision.item.completionReview)delete item.completionReview;saveOperationalData();return decision.item;};
      const linkPlan=async(planId,sourceVisitId,date)=>{const current=operationalRecord(item.id),schedule=[...current.installationSchedule,slot(planId,date)].map(entry=>entry.workPlanId===planId?{...entry,sourceVisitId}:entry),validation=validateContinuationPlanRelations(current,schedule);if(!validation.ok)throw new Error('Continuation relation failed: '+validation.code);const next=structuredClone(current);next.installationSchedule=schedule;applyContinuationPlanRelations(next,validation,'2026-09-17T13:00:00.000Z');Object.assign(item,next);saveOperationalData();return operationalRecord(item.id);};
      const first=await submitVisit('plan-1','2026-09-17'),firstReview=structuredClone(first.completionReview),firstState=resolveOperationalState(first,{today:'2026-09-17'});
      await continuation(firstReview.targetVisitId);await linkPlan('plan-2',firstReview.targetVisitId,'2026-09-18');
      const second=await submitVisit('plan-2','2026-09-18'),secondReview=structuredClone(second.completionReview),secondState=resolveOperationalState(second,{today:'2026-09-18'});
      await continuation(secondReview.targetVisitId);await linkPlan('plan-3',secondReview.targetVisitId,'2026-09-19');
      const third=await submitVisit('plan-3','2026-09-19'),thirdReview=structuredClone(third.completionReview),thirdState=resolveOperationalState(third,{today:'2026-09-19'});
      const approved=applyCompletionReviewDecision(third,'approve',{targetVisitId:thirdReview.targetVisitId,submissionRevision:thirdReview.submissionRevision,actor:'Supervisor',timestamp:'2026-09-19T13:00:00.000Z'});if(!approved.ok)throw new Error('Approval failed: '+approved.code);Object.assign(item,approved.item);if(!approved.item.completionReview)delete item.completionReview;saveOperationalData();
      const final=operationalRecord(item.id),history=final.completionReviewHistory||[];askConfirm=originalAskConfirm;installations=installations.filter(record=>record.id!==${fixtureId});saveOperationalData();render();
      return {first:{visitId:firstReview.targetVisitId,review:firstReview,status:firstReview.status,state:{signal:firstState.primarySignal,action:firstState.nextAction}},second:{visitId:secondReview.targetVisitId,review:secondReview,status:secondReview.status,state:{signal:secondState.primarySignal,action:secondState.nextAction}},third:{visitId:thirdReview.targetVisitId,review:thirdReview,status:thirdReview.status,state:{signal:thirdState.primarySignal,action:thirdState.nextAction}},distinct:new Set([firstReview.targetVisitId,secondReview.targetVisitId,thirdReview.targetVisitId]).size,history:history.map(event=>({type:event.type,targetVisitId:event.targetVisitId})),final:{stage:final.workflowStage,status:final.status,progress:final.progress,completedAt:final.completedAt,review:Boolean(final.completionReview),signal:resolveOperationalState(final,{today:'2026-09-19'}).primarySignal}};
    })()`);
    assert.equal(result.first.review.status,'pending');assert.equal(result.first.state.signal,'ACTION_REQUIRED');assert.equal(result.first.state.action,'REVIEW_COMPLETION_SUBMISSION');
    assert.equal(result.second.review.status,'pending');assert.equal(result.second.state.signal,'ACTION_REQUIRED');assert.equal(result.second.state.action,'REVIEW_COMPLETION_SUBMISSION');
    assert.equal(result.third.review.status,'pending');assert.equal(result.third.state.signal,'ACTION_REQUIRED');assert.equal(result.third.state.action,'REVIEW_COMPLETION_SUBMISSION');
    assert.equal(result.distinct,3);assert.deepEqual(result.history.map(event=>event.type),['completion-submitted','continuation-required','completion-submitted','continuation-required','completion-submitted','completion-approved']);
    assert.deepEqual(result.history.map(event=>event.targetVisitId),[result.first.visitId,result.first.visitId,result.second.visitId,result.second.visitId,result.third.visitId,result.third.visitId]);
    assert.equal(result.final.stage,'completed');assert.equal(result.final.status,'Tamamlandı');assert.equal(result.final.progress,100);assert.ok(result.final.completedAt);assert.equal(result.final.review,false);assert.equal(result.final.signal,'COMPLETED');
  } finally {
    await protocol.evaluate(`(()=>{askConfirm=async(message,options={})=>(await showActionDialog({...options,message}))!==null;['#serviceEntryDialog','#standardActionDialog'].forEach(selector=>{const dialog=document.querySelector(selector);if(dialog?.open)dialog.close()});installations=installations.filter(record=>record.id!==${fixtureId});saveOperationalData();render()})()`).catch(()=>{});
    if(await protocol.evaluate(`!document.querySelector('#appView').classList.contains('hidden')`).catch(()=>false))await logout();
  }
...
test('Faz 7.3F planlama UI context, exact read-only ziyaret ve güvenli geri dönüş sağlar', async () => {
  const fixtureId=-975042;
  const result=await protocol.evaluate(`(async()=>{const previousUser=currentUser,supervisor=userDirectory.find(user=>user.role==='supervisor'),item={id:${fixtureId},revision:1,customer:'CONTINUATION UI',salesOrderNumber:'SO-73F-B',workflowStage:'inService',status:'Devam ediyor',progress:80,pendingContinuationPlanning:false,orderProducts:[],installationSchedule:[{id:'target-slot',workPlanId:'target-plan',date:'2026-09-17',startTime:'09:00',endTime:'11:00',technicians:[supervisor.name],activityType:'installation',productControl:'required',checklistRequirement:'required'},{id:'future-slot',workPlanId:'future-plan',workPlanNote:'Korunacak not',date:'2099-09-21',startTime:'10:00',endTime:'12:00',technicians:[supervisor.name],activityType:'continuation',productControl:'required',checklistRequirement:'required'}],serviceVisits:[{visitId:'source-visit',workPlanId:'target-plan',actualVisitDate:'2026-09-17',serviceOutcome:'installationCompleted',completedWork:'Tamamlanan işler',remainingWork:'Kalan işler',technicians:[supervisor.name],technicianEntries:[{name:supervisor.name,travelDuration:0,travelUnit:'Saat',siteDuration:2,siteUnit:'Saat'}]}],completionReviewHistory:[{type:'continuation-required',targetVisitId:'source-visit',actor:supervisor.name,timestamp:'2026-09-17T12:00:00.000Z',decisionNote:'Tekrar ziyaret gerekli.'}]};currentUser=supervisor;installations.push(item);saveOperationalData();openPlanning(item.id);const context=document.querySelector('#planningContinuationContext'),opened={visible:!context.classList.contains('role-hidden'),note:context.textContent.includes('Tekrar ziyaret gerekli.'),choices:[...document.querySelectorAll('#continuationExistingPlanSelect option')].map(option=>option.value),autoSource:planningScheduleDraft.find(slot=>slot.workPlanId==='future-plan')?.sourceVisitId||''};document.querySelector('#continuationExistingPlanSelect').value='future-plan';context.querySelector('[data-link-continuation-plan]').click();const selected=planningScheduleDraft.find(slot=>slot.workPlanId==='future-plan')?.sourceVisitId||'';context.querySelector('[data-open-continuation-source-visit]').click();const service=document.querySelector('#serviceEntryDialog'),form=document.querySelector('#serviceEntryForm'),readonly={planningOpen:document.querySelector('#planningDialog').open,serviceOpen:service.open,index:document.querySelector('#serviceVisitIndex').value,plan:document.querySelector('#serviceWorkPlanId').value,mode:form.dataset.continuationReadOnly,actualVisitDate:document.querySelector('#serviceActualDate').value,allMutationsDisabled:[...form.querySelectorAll('input,select,textarea,button')].filter(control=>!control.matches('[value="cancel"],#toggleServiceFullscreen')).every(control=>control.disabled),addTechnicianDisabled:document.querySelector('#addServiceTechnician').disabled,createReportDisabled:document.querySelector('#createServiceReport').disabled,uploadDisabled:form.elements.serviceAttachments.disabled,submitHidden:form.querySelector('.primary-button[value="default"]').classList.contains('role-hidden')};service.close();await new Promise(resolve=>setTimeout(resolve,30));const returned=document.querySelector('#planningDialog').open,restored={mode:form.dataset.continuationReadOnly||'',createReportDisabled:document.querySelector('#createServiceReport').disabled,uploadDisabled:form.elements.serviceAttachments.disabled};document.querySelector('#planningDialog').close();installations=installations.filter(record=>record.id!==item.id);saveOperationalData();currentUser=previousUser;render();return{opened,selected,readonly,returned,restored};})()`);
  assert.deepEqual(result.opened,{visible:true,note:true,choices:['','future-plan'],autoSource:''});assert.equal(result.selected,'source-visit');assert.deepEqual(result.readonly,{planningOpen:false,serviceOpen:true,index:'0',plan:'target-plan',mode:'true',actualVisitDate:'2026-09-17',allMutationsDisabled:true,addTechnicianDisabled:true,createReportDisabled:true,uploadDisabled:true,submitHidden:true});assert.equal(result.returned,true);assert.deepEqual(result.restored,{mode:'',createReportDisabled:false,uploadDisabled:false});
});

test('Faz 7.3F Scenario A UI kaydı sourceVisitId döngüsünü ve diğer açık ziyaretleri korur', async () => {
  const fixtureId=-975043;
  const result=await protocol.evaluate(`(async()=>{const previousUser=currentUser,supervisor=userDirectory.find(user=>user.role==='supervisor'),item={id:${fixtureId},revision:1,customer:'CONTINUATION SAVE',salesOrderNumber:'SO-73F-A',workflowStage:'inService',status:'Devam planı bekliyor',progress:80,pendingContinuationPlanning:true,orderProducts:[],installationSchedule:[{id:'target-slot',workPlanId:'target-plan',date:'2026-09-17',startTime:'09:00',endTime:'11:00',technicians:[supervisor.name],activityType:'installation',productControl:'required',checklistRequirement:'required'}],serviceVisits:[{visitId:'source-visit',workPlanId:'target-plan',actualVisitDate:'2026-09-17',serviceOutcome:'installationCompleted'},{visitId:'legacy-open',workPlanId:'target-plan',actualVisitDate:'2026-09-16',serviceOutcome:'couldNotPerform'}],completionReviewHistory:[{type:'continuation-required',targetVisitId:'source-visit',actor:supervisor.name,timestamp:'2026-09-17T12:00:00.000Z',decisionNote:'Yeni plan gerekli.'}]};currentUser=supervisor;installations.push(item);saveOperationalData();openPlanning(item.id);const continuationGroup=[...document.querySelectorAll('#planningScheduleRows [data-planning-group]')].find(group=>group.dataset.sourceVisitId==='source-visit'),planId=continuationGroup?.dataset.planningGroup||'';continuationGroup.querySelector('[data-group-field="date"]').value='2099-09-24';continuationGroup.querySelector('[data-slot-field="startTime"]').value='09:00';continuationGroup.querySelector('[data-slot-field="endTime"]').value='11:00';continuationGroup.querySelector('[data-slot-field="technician"]').value=supervisor.name;const form=document.querySelector('#planningForm');form.dispatchEvent(new SubmitEvent('submit',{bubbles:true,cancelable:true,submitter:form.querySelector('.primary-button[value="default"]')}));await new Promise(resolve=>setTimeout(resolve,160));const saved=operationalRecord(item.id),plan=serviceWorkPlans(saved).find(entry=>entry.id===planId),state=resolveOperationalState(saved,{today:'2026-09-17'}),response={dialogOpen:document.querySelector('#planningDialog').open,planId,slotSources:plan?.slots.map(slot=>slot.sourceVisitId),readSource:resolveContinuationSourceVisit(saved,plan)?.visitId,sourcePlanned:Boolean(saved.serviceVisits.find(visit=>visit.visitId==='source-visit')?.continuationPlannedAt),legacyPlanned:Boolean(saved.serviceVisits.find(visit=>visit.visitId==='legacy-open')?.continuationPlannedAt),pending:saved.pendingContinuationPlanning,state:{signal:state.primarySignal,action:state.nextAction}};if(document.querySelector('#planningDialog').open)document.querySelector('#planningDialog').close();installations=installations.filter(record=>record.id!==item.id);saveOperationalData();currentUser=previousUser;render();return response;})()`);
  assert.equal(result.dialogOpen,false);assert.ok(result.planId);assert.deepEqual(result.slotSources,['source-visit']);assert.equal(result.readSource,'source-visit');assert.equal(result.sourcePlanned,true);assert.equal(result.legacyPlanned,false);assert.equal(result.pending,true);assert.deepEqual(result.state,{signal:'BLOCKED',action:'CREATE_CONTINUATION_PLAN'});
});

test('Completion Review pending resubmit en son ilgili düzeltme talebini ve continuation metinlerini gösterir', async () => {
  const result=await protocol.evaluate(`(() => {
    const originalUser=currentUser,originalLanguage=language,supervisor=userDirectory.find(user=>user.role==='supervisor');currentUser=supervisor;language='tr';
    const visit={visitId:'target-visit',workPlanId:'target-plan',actualVisitDate:'2026-09-17',serviceOutcome:'installationCompleted'},otherVisit={visitId:'other-visit',workPlanId:'other-plan',actualVisitDate:'2026-09-16',serviceOutcome:'installationCompleted'},plans=[{id:'target-slot',workPlanId:'target-plan',date:'2026-09-17',startTime:'09:00',endTime:'11:00',technicians:[supervisor.name]},{id:'linked-slot',workPlanId:'linked-plan',sourceVisitId:'target-visit',date:'2026-09-26',startTime:'09:00',endTime:'17:00',technicians:[supervisor.name]}],base={id:-975050,revision:1,workflowStage:'inService',orderProducts:[],installationSchedule:plans,serviceVisits:[visit,otherVisit]},submitted={type:'completion-submitted',targetVisitId:'target-visit',actor:'Teknisyen A',timestamp:'2026-09-17T09:00:00.000Z',submissionRevision:1},requestA={type:'revision-requested',targetVisitId:'target-visit',actor:'Supervisor A',timestamp:'2026-09-17T10:00:00.000Z',submissionRevision:1,decisionCategory:'missing-technical-document',decisionNote:'Raporu ekleyerek tekrar gönderin.'},resubmit2={type:'completion-resubmitted',targetVisitId:'target-visit',actor:'Teknisyen A',timestamp:'2026-09-17T11:00:00.000Z',submissionRevision:2},otherRequest={type:'revision-requested',targetVisitId:'other-visit',actor:'Supervisor B',timestamp:'2026-09-17T11:30:00.000Z',submissionRevision:1,decisionCategory:'work-description',decisionNote:'Başka ziyaret notu.'},requestB={type:'revision-requested',targetVisitId:'target-visit',actor:'Supervisor C',timestamp:'2026-09-17T12:00:00.000Z',submissionRevision:2,decisionCategory:'work-description',decisionNote:'İkinci düzeltme talebi.'},resubmit3={type:'completion-resubmitted',targetVisitId:'target-visit',actor:'Teknisyen A',timestamp:'2026-09-17T13:00:00.000Z',submissionRevision:3},initial={...base,completionReview:{targetVisitId:'target-visit',status:'pending',submissionRevision:1,submittedBy:'Teknisyen A',submittedAt:'2026-09-17T09:00:00.000Z'},completionReviewHistory:[submitted]},revision2={...base,completionReview:{targetVisitId:'target-visit',status:'pending',submissionRevision:2,submittedBy:'Teknisyen A',submittedAt:'2026-09-17T11:00:00.000Z'},completionReviewHistory:[submitted,requestA,resubmit2,otherRequest]},revision3={...base,completionReview:{targetVisitId:'target-visit',status:'pending',submissionRevision:3,submittedBy:'Teknisyen A',submittedAt:'2026-09-17T13:00:00.000Z'},completionReviewHistory:[submitted,requestA,resubmit2,otherRequest,requestB,resubmit3]},before=JSON.stringify(revision3.completionReviewHistory),initialCard=completionReviewCardMarkup(initial),request2=completionReviewPreviousRevisionRequest(revision2,revision2.completionReview),card2=completionReviewCardMarkup(revision2),request3=completionReviewPreviousRevisionRequest(revision3,revision3.completionReview),card3=completionReviewCardMarkup(revision3),linkedMarkup=continuationContextMarkup(base,{visit,decision:null,linkedPlans:[serviceWorkPlans(base)[1]],pending:false}),choiceMarkup=continuationContextMarkup({...base,installationSchedule:[...plans,{id:'future-slot',workPlanId:'future-plan',date:'2099-09-27',startTime:'09:00',endTime:'11:00',technicians:[supervisor.name]}]},{visit,decision:null,linkedPlans:[],pending:true});currentUser=originalUser;language=originalLanguage;return{initialHidden:!completionReviewPreviousRevisionRequest(initial,initial.completionReview)&&!initialCard.includes('ÖNCEKİ DÜZELTME TALEBİ'),revision2:{category:request2?.decisionCategory,note:request2?.decisionNote,actor:request2?.actor,hasTitle:card2.includes('ÖNCEKİ DÜZELTME TALEBİ'),hasReason:card2.includes('Teknik rapor/belge eksik'),hasNote:card2.includes('Raporu ekleyerek tekrar gönderin.'),hasOther:card2.includes('Başka ziyaret notu.')},revision3:{category:request3?.decisionCategory,note:request3?.decisionNote,actor:request3?.actor,hasLatest:card3.includes('İkinci düzeltme talebi.'),hasOld:card3.includes('Raporu ekleyerek tekrar gönderin.')},historyUnchanged:before===JSON.stringify(revision3.completionReviewHistory),continuation:{friendly:linkedMarkup.includes('Devam planı: 26 Eyl 2026 · Çalışma Planı 2'),noIso:!linkedMarkup.includes('2026-09-26'),choice:choiceMarkup.includes('Seçili planı devam planı yap')}};
  })()`);
  assert.deepEqual(result,{initialHidden:true,revision2:{category:'missing-technical-document',note:'Raporu ekleyerek tekrar gönderin.',actor:'Supervisor A',hasTitle:true,hasReason:true,hasNote:true,hasOther:false},revision3:{category:'work-description',note:'İkinci düzeltme talebi.',actor:'Supervisor C',hasLatest:true,hasOld:false},historyUnchanged:true,continuation:{friendly:true,noIso:true,choice:true}});
});
