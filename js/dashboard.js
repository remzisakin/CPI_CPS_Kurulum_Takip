...
function dashboardWeekRange(){const now=new Date(),start=new Date(now);start.setHours(0,0,0,0);start.setDate(start.getDate()-((start.getDay()+6)%7));const end=new Date(start);end.setDate(end.getDate()+7);return{start:start.toISOString().slice(0,10),end:end.toISOString().slice(0,10)}}
function dashboardPlans(item){return serviceWorkPlans(item).filter(plan=>currentUser?.role!=='technician'||plan.slots.some(slot=>(slot.technicians||[]).includes(currentUser.name)))}
function dashboardIssues(records){
  ...
  records.forEach(item=>{
    ...
    if(currentUser.role==='technician'){const today=new Date().toISOString().slice(0,10),pending=dashboardPlans(item).find(plan=>plan.date<=today&&!serviceVisitForPlan(item,plan.id).visit);if(pending)add(item,'Servis kaydı bekliyor',`${pending.date} tarihli çalışma için kayıt girilmedi.`)}
  });
  return issues;
}
...
function renderDashboard(){
  if(!currentUser||!$('#dashboardView'))return;const records=dashboardRecordsForUser(),active=records.filter(item=>!['draft','completed'].includes(item.workflowStage)),range=dashboardWeekRange(),weekRecords=records.filter(item=>dashboardPlans(item).some(plan=>plan.date>=range.start&&plan.date<range.end)),issues=dashboardIssues(records),overruns=records.filter(item=>item.serviceOverrun||item.status==='Süre aşıldı'),today=new Date().toISOString().slice(0,10),agenda=records.flatMap(item=>dashboardPlans(item).filter(plan=>plan.date===today).map(plan=>({item,plan,start:plan.slots.map(slot=>slot.startTime||'09:00').sort()[0]||'09:00',people:planningSlotNames(plan.slots)}))).sort((a,b)=>a.start.localeCompare(b.start));
