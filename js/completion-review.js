...
function completionReviewPreviousRevisionRequest(item,review){
  const active=normalizeCompletionReview(review);if(!active||active.status!=='pending'||active.submissionRevision<2)return null;
  const history=Array.isArray(item?.completionReviewHistory)?item.completionReviewHistory:[],target=active.targetVisitId,resubmissionIndex=history.findLastIndex(event=>event?.type==='completion-resubmitted'&&String(event.targetVisitId||'').trim()===target&&Number(event.submissionRevision)===active.submissionRevision);
  if(resubmissionIndex<0)return null;
  const previous=history.slice(0,resubmissionIndex).reverse().find(event=>event?.type==='revision-requested'&&String(event.targetVisitId||'').trim()===target&&Number(event.submissionRevision)===active.submissionRevision-1);
  return previous&&typeof previous.decisionCategory==='string'&&previous.decisionCategory.trim()&&typeof previous.decisionNote==='string'&&previous.decisionNote.trim()&&typeof previous.actor==='string'&&previous.actor.trim()&&typeof previous.timestamp==='string'&&previous.timestamp.trim()?previous:null;
}
function completionReviewPreviousRevisionMarkup(event){
  if(!event)return'';
  return`<aside class="completion-review-previous-revision"><span>ÖNCEKİ DÜZELTME TALEBİ</span><strong>${escapeHtml(completionReviewRevisionReasonLabel(event.decisionCategory))}</strong><p>${escapeHtml(event.decisionNote)}</p><small>${escapeHtml([event.actor,completionReviewDateTime(event.timestamp)].filter(Boolean).join(' · '))}</small></aside>`;
}
...
function completionReviewCardMarkup(item){
  ...
  const canDecide=info.key==='pending'&&hasRole('supervisor'),canRevise=info.key==='revision'&&completionReviewRevisionAuthorized(item),revision=Number(item.revision)||1,previousRevisionRequest=canDecide?completionReviewPreviousRevisionRequest(item,review):null;
  return `<section class="detail-section completion-review-section" data-completion-review-status="${escapeHtml(info.key)}">
    ...
    ${completionReviewPreviousRevisionMarkup(previousRevisionRequest)}
...
Object.assign(englishUi,{
  'Kapanış Kaydını Düzelt':'Edit Closure Record','Düzelt ve Tekrar Gönder':'Revise and Resubmit','DÜZELTME TALEBİ':'REVISION REQUEST','ÖNCEKİ DÜZELTME TALEBİ':'PREVIOUS REVISION REQUEST','Seçili planı devam planı yap':'Make selected plan the continuation plan'
});
