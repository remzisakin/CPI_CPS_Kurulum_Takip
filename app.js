...
let installations = (JSON.parse(localStorage.getItem('cps-installations') || 'null') || seedInstallations).map(item=>{const sample=seedInstallations.find(seed=>seed.id===item.id);const workflowStage=item.workflowStage||(item.status==='Taslak'?'draft':item.status==='Planlama bekliyor'?'awaitingPlanning':'planned');return {...item,workflowStage,salesOrderNumber:item.salesOrderNumber||item.so,requestDate:item.requestDate||sample?.requestDate||'—',salesEngineer:item.salesEngineer||sample?.salesEngineer||'Atama bekliyor'}});
...
function openFullEdit(id){
  ...
  setFormValue(form,'customer',item.customer);setFormValue(form,'customerType',item.customerType||'direct');updateCustomerFields();setFormValue(form,'mtbName',item.mtbName);setFormValue(form,'endUserName',item.endUserName);setFormValue(form,'salesEngineer',item.salesEngineer);setFormValue(form,'salesOrderNumber',item.salesOrderNumber);setFormValue(form,'ptd',item.ptd==='—'?'':item.ptd);setFormValue(form,'amount',item.installationAmount);setFormValue(form,'requestDate',item.requestDateIso||new Date().toISOString().slice(0,10));setFormValue(form,'address',item.address);
...
function sendToPlanning(id){if(currentUser?.role!=='sales'){showToast('Bu işlemi yalnızca Satış Mühendisi yapabilir.');return}if(!navigator.onLine){showToast('İşleme devam etmek için online olun.');return}const item=installations.find(record=>record.id===id);if(!item||item.workflowStage!=='draft')return;item.workflowStage='awaitingPlanning';item.status='Planlama bekliyor';item.date='Planlama bekliyor';item.sentToPlanningAt=new Date().toISOString();localStorage.setItem('cps-installations',JSON.stringify(installations));render();showToast('Kurulum Servis Supervisor planlama kuyruğuna gönderildi.')}
...
$('#installationForm').addEventListener('submit',event=>{
  ...
  else{installations.unshift({id:Date.now(),...editable,attachments,workflowStage:'draft',status:'Taslak',date:'—',tech:'Atama bekliyor',initials:'?',plannedDuration:null,plannedUnit:'Saat',progress:0,createdBy:currentUser.username,createdAt:new Date().toISOString()});successMessage='Yeni kurulum taslak olarak kaydedildi.'}
