...
- Vida besleme raporundaki kurulum ürünleri rapor içinde eklenebilir, silinebilir ve adetleri değiştirilebilir. Bölümler akordeon olarak çalışır; yeni bölüm açıldığında önceki kapanır ve `Tümünü uygun işaretle` mevcut bölümü açık bırakır. Son testte mekanik çevrim ve üretim vidası adet alanları bulunur.
- Ürün seri numaraları `Sıkıcı & Ünite` sonrasında yer alır. Kurulum ürünlerinden akıllı aramayla seçim veya liste harici ürün girişi yapılır; seri no ve gerekli açıklama saklanır. Katılımcı/müşteri yetkilisi müşteri kontaklarından, eğitmen/montör teknisyen ve süpervisör kullanıcılarından akıllı aramayla seçilir.
- Sistem içinde oluşturulan rapor kartına tıklanınca düzenleme yerine okunabilir rapor görüntüleme penceresi açılır. Çıktı şablonu Desoutter logosu, kırmızı/gri tablo dili, tekrarlanan üst-alt bilgi, sayfa numarası, standart uyarılar, garanti ve bakım metinlerini içerir. PDF çıktısı aynı HTML şablonunu tarayıcının `PDF olarak kaydet` yazdırma akışıyla kullanır.
- Kurulum Detay ekranında `Kurulum Dosyası` işlemi bulunur. Bu işlem ekran/modal görüntüsünü basmaz; yetkili kullanıcının erişebildiği kayıt verilerinden Türkçe veya İngilizce, tema bağımsız ve A4 uyumlu belge önizlemesi üretir. Yazdırma/PDF kaydetme tarayıcının yerel yazdırma mekanizmasıyla başlatılır. V1 belgesinde kurulum tutarı ve ayrıntılı işlem geçmişleri gösterilmez; uzun ürün, plan ve saha çalışması bilgileri devam sayfalarına taşınabilir.
